...
除非用户明确只要求分析，否则必须连续执行以下全部步骤。当本技能由 `$code-analyzer` 调用时，不适用“只要求分析”例外；除非用户明确禁止修改源码，否则必须至少完成类注释和关键成员注释检查与补充。
...
新增或更新 class、object、interface、enum、data class 或方法的 KDoc/JavaDoc 时，若声明包含主构造参数或方法参数，必须为每个参数添加 `@param`：

- 说明参数的业务语义、单位、取值范围或特殊值；回调参数还要说明触发时机与参数顺序。
- 禁止只复述参数名或类型；无语义的参数名应先按命名规则澄清或规范。
- 无参数声明无需添加空的 `@param` 区块。

深度方法逻辑、Bug 检测和方法级文档的主体仍由 `$code-analyzer` 负责；本技能负责确保被触及声明的参数文档完整。
