---
name: code-normalize
description: 检测并安全规范 Java、Kotlin 类中的成员变量命名，更新全部引用，补充缺失的类注释，并为关键成员添加作用说明。当用户要求检查或重构 bname、a_b、mUserName、含义不清的缩写、snake_case 成员，或希望在不改变行为的前提下提升类与成员命名、注释质量时使用。
---
