...
- 必须遵循 `$skill-common` 的基础规范，并使用中文向用户说明执行过程与结果。
- Android CLI 的项目创建、SDK、设备、模拟器、布局检查和环境诊断必须使用本技能，禁止改走其他 CLI 流程。
...
任务完成并验证后，必须调用 `$skill-common` 复盘本技能；本技能不重复保存进化门槛和去重策略。
