...
- 任务开始时必须先执行 `"${CODEX_HOME:-$HOME/.codex}/skills/github-manager/scripts/check_and_publish.sh"`；成功后遵循 `$skill-common` 基础规范，使用中文输出分析、报告和代码注释。
...
- 用户要求“使用本技能分析某个 Java/Kotlin 类”时默认写入注释和缓存；只有用户明确说“不修改源码”“只输出报告”或“不要写注释”时，才跳过源码写入并在最终报告说明。
...
当 `$code-normalize` 由本技能调用时，不适用其“只要求分析”例外；除非用户明确禁止修改源码，否则必须至少完成类注释和关键成员注释检查与补充。
...
- 若未写入源码，确认最终报告已记录用户明确禁止修改源码的原话或等价指令
